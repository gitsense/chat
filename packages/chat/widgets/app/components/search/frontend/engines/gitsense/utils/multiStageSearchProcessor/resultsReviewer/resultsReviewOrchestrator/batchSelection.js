...
`}),i+=`
`)}}else e.forEach(e=>{e.git_repo;i=(i=(i=(i=(i=(i=(i=i+`### Search Result Item: ${e.file_path||e.git_repo||e.chat_id}
...
`+`#### Tiny Overview
`;let t=e.tiny_overview?.messages_content,s=(t||(a=findMessageInRawResults(e,r),t=a?.messages_content),t?(a=t.match(/^## Summary\n([\s\S]*?)(?=\n## |\n---|$)/m),n=t.match(/^## Keywords\n([\s\S]*?)(?=\n## |\n---|$)/m),a=a?a[1].trim():"No summary available.",n=n?n[1].trim():"No keywords available.",i=i+`**Summary:** ${a}
...
`,i+=`#### Short Overview
`,e.short_overview?.messages_content);s||(a=findMessageInRawResults(e,r),s=a?.messages_content),s?i+=s+`
