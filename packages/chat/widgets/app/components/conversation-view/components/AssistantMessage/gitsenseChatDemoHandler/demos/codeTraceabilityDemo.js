...
let{CodeBlockUtils,ChatUtils,MessageUtils}=require("@gitsense/gsc-utils"),MessageService=require("../../../../services/MessageService"),DEMO_CONSTANTS=require("../constants"),DemoUtils=require("../utils/demoUtils"),DEMO_TITLE="## Traceable Code Demo",GENERATE_HELLO_WORLD_COMMAND='Generate a "Hello World" program in JavaScript. Before generating the code, acknowledge that you have read the instructions in the system prompt regarding code generation and code response requirements.',CHANGE_HELLO_TO_HEY_COMMAND='Change "Hello" to "Hey" in the JavaScript program and provide a patch. Before generating the patch, acknowledge that you have read the instructions in the system prompt regarding code response requirements and patch generation.',codeTraceabilityDemo={id:"code-traceability",name:"Traceable Code",description:"Learn how GitSense Chat generates code and provides unparalleled traceability for every AI-assisted change.",scenes:[{id:"code-traceability-intro",action:"typeAndAppend",triggerCondition:(e,t,o,a)=>e.message.includes(DEMO_TITLE),contentToType:`
### Welcome!

In this demo, you'll learn how GitSense Chat ensures every line is traceable back to its conversational origin, complete with unique \`Block-UUID\`s, versioning, and authorship. This allows you to audit AI contributions and understand the full human-AI collaboration.
...
> **Chat Input Disabled:** The chat input is currently disabled, but will re-enable once the demo has finished.
            `,pauseAfterTypingMs:DEMO_CONSTANTS.SCENE_TRANSITION_PAUSE_MS,postProcess:async(e,t,o,a)=>{var i=Array.from(o.querySelectorAll("p")).find(e=>e.textContent.includes("LLM Responses are Live")),i=(i&&(i.style.border="1px solid #856404",i.style.borderRadius="5px",i.style.backgroundColor="#fff3cd",i.style.padding="15px",i.style.marginBottom="20px"),o.querySelector("a"));i?(await DemoUtils.simulateTyping(GENERATE_HELLO_WORLD_COMMAND,a),i.onclick=e=>{e.preventDefault(),DemoUtils.sendMessage()}):console.error(`No send link in current scene ${t.id} found`)},nextSceneId:"code-traceability-await-hello-world"},{id:"code-traceability-await-hello-world",action:"createBlankChildMessage",triggerCondition:(e,t,o,a)=>{var i,o=MessageUtils.getMessageById(o.chat,e.parent_id);return!(!o||o.message.trim()!==GENERATE_HELLO_WORLD_COMMAND||(o=CodeBlockUtils.extractCodeBlocks(e.message,{silent:!0}).blocks,(e=o.find(e=>"code"===e.type&&"javascript"===e.language)).headerParseError?(o=h.createH3({text:"Header Error",style:{color:"#721c24",marginTop:"0",fontSize:"16px"}}),i=h.createParagraph({text:e.headerParseError,style:{color:"#721c24"}}),o=h.createDiv({cls:"patch-error-container",append:[o,i],style:{marginTop:"5px",marginBottom:"20px",padding:"15px",paddingBottom:"0px",border:"1px solid #dc3545",borderRadius:"4px",backgroundColor:"#f8d7da"}}),t.appendChild(o),1):!e))},nextSceneId:"code-traceability-explain-hello-world"},{id:"code-traceability-explain-hello-world",action:"typeAndAppend",contentToType:`
## What Happened
