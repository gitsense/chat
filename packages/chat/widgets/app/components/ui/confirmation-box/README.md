...
Block-UUID: 6df2a814-d0b2-4ddd-be88-781b01b9f646
Parent-UUID: 849b3be5-351c-4104-9934-8f4e554c798c
Version: 1.2.0
Description: README documentation for the reusable Confirmation Box component.
Language: markdown
Created-at: 2025-09-10T00:29:56.316Z
Authors: Gemini 2.5 Flash Thinking (v1.0.0), Gemini 2.5 Flash (v1.1.0), GLM-4.7 (v1.2.0)
-->
...
// Example with onCancel callback
myConfirmationBox.show(
  {
  ...
  () => {
    console.log("User cancelled or chose not to save");
...
*   `onCancel` (Optional `function`): A callback function that will be executed when the user cancels the dialog by clicking the "Cancel" button, the close button (×), or clicking outside the modal on the overlay. This function receives no arguments.
...
*   **Cancel Callback:** The `show` method supports an optional `onCancel` callback that is executed when the user dismisses the dialog via cancel actions (Cancel button, close button, or overlay click).
