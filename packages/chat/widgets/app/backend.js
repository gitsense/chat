...
`),l.end(),await H(t,j);void await 0}else if(u){var K=c;var X=S;let a=[],e=(X.forEach((e,t)=>{t!==X.length-1&&({role:t,message:e}=e,""!==e)&&a.push({role:t,content:e})}),Date.now()),t="This is a GitSense Notes response.",s={content:t,done:!1};l.write(`id: ${e}
event: message
data: ${JSON.stringify(s)}
...
`),l.end(),await H(t,K);void await 0}else{if(J){var Z=S,T=w,A=c,Q=d,N=n,ee=t,te=h,ae=f;let a=null,s=[];Z.forEach((e,t)=>{t!==Z.length-1&&({role:t,message:e}=e,""!==e)&&(t===SYSTEM_ROLE?a=e:s.push({role:t,content:e}))});var I={messages:I=populateMessages(A,s),model:N,temperature:Q,max_tokens:ee,stream:!0};a&&(N=populateMessages(A,[{content:a}]),I.system=N[0].content);try{o.setTimeout(3e4);var R=Date.now();let e=!1;o.on("close",()=>{console.log("Client closed connection"),e=!0});var D,C,se,re=new Anthropic({apiKey:te}),M=(console.log(`Starting stream ${ae}...`),{startedAt:(new Date).getTime()}),ie=await re.messages.create(I);T.createdAt=Date.now(),T.payload=I;let t=!1,a="";for await(D of ie){if(T.updatedAt=Date.now(),e)break;if("content_block_delta"===D.type&&(C=D.delta.text||"")&&(a+=C,se={content:C,done:!1},l.write(`id: ${R}
event: message
...
`),l.end()),await H(a,A,M)}catch(e){console.error("Streaming error: "+e),await k(l,e.message),l.end()}}else{var ne=e,oe=S,O=w,L=c,le=d,de=n,ce=t,me=s,ue=i,he=h,ge=f;let a=[],r=(oe.forEach((e,t)=>{t!==oe.length-1&&({role:t,message:e}=e,""!==e)&&a.push({role:t,content:e})}),populateMessages(L,a));try{o.setTimeout(3e4);let t=!1;o.on("close",e=>{console.log("Client closed connection"),t=!0});var U={model:de,temperature:le,messages:r,stream:!0,max_tokens:ce},pe=((me||ue)&&(U.extra_body={google:{thinking_config:{thinking_budget:0,include_thoughts:ue}}}),new OpenAI({apiKey:he,baseURL:ne.baseURL})),F={startedAt:(new Date).getTime()};let e=null;try{console.log(`Starting stream ${ge}...`),e=await pe.chat.completions.create(U)}catch(e){return void await(console.error("Failed to create a stream:",e),e.message.includes("status code")&&(e.message+="\n\nPlease look up the error code with the provider to determine the problem."),await k(l,e.message,L),void l.end())}O.createdAt=Date.now(),O.payload=U;var $,ye=Date.now();let a=!1,s="";for await($ of e){if(O.updatedAt=Date.now(),t)break;var b=$.choices[0]?.delta?.content||"",x=$.choices[0]?.finish_reason,fe=x&&"null"!==x&&"stop"===x,Ee={content:b,done:fe};if(b&&(s+=b),l.write(`id: ${ye}
event: message
data: ${JSON.stringify(Ee)}

`),l.flush?.(),fe){console.log("Finished streaming "+ge),""===s.trim()&&(s="### WARNING\n\nThe LLM stopped without returning a response. If a response was expected, please try sending a `Try again` message."),F.done=!0,F.stoppedAt=(new Date).getTime(),a=!0;break}}a||(F.incomplete=!0,F.stoppedAt=(new Date).getTime(),s+="\n---\nWarning: Incomplete response"),t||(l.write(`id: ${ye}
event: complete
data: {}

`),l.end()),await H(s,L,F)}catch(e){console.error("Streaming error:",e),await k(l,e.message),l.end()}}await 0}}}finally{console.log("Deleting streaming lock "+f),delete global.gschat.streaming[f]}}else l.status(500).send({status:"failed",data:`Server side error. No model identifier for ${c} by ${a} found.`})}else l.status(400).send({status:"failed",data:"No providers for the LLM "+c})}else await k(l,`No ${a} API key defined. Please define to chat with ${c}.`)}else l.status(400).send({status:"failed",data:{provider:"Missing provider information"}})}}finally{console.log("Closing db"),await closeAsync(v)}async function k(e,t,a){console.log("Stream error message",t);try{try{e.setHeader("Content-Type","text/event-stream"),e.setHeader("Cache-Control","no-cache"),e.setHeader("Connection","keep-alive"),e.flushHeaders()}catch(e){}var s=Date.now(),r={content:t,done:!1};e.write(`id: ${s}
event: message
data: ${JSON.stringify(r)}

`),e.flush?.(),r={content:"",done:!0},e.write(`id: ${s}
event: message
data: ${JSON.stringify(r)}

`),e.flush?.(),e.write(`id: ${s}
event: complete
data: {}

`),e.end(),await H(t,a),console.log("saved response")}catch(e){console.log(e.message)}}async function H(t,a,e){var s=new RegExp("Authored by [^\n]+\n*$");let r=extractCodeBlocks(t,{silent:!0}).blocks||[];for(let e=0;e<r.length;e++){var i=r[e];i.header?.["Block-UUID"]&&(i=removeLineNumbers(i.headerText+"\n\n\n"+i.content),t=updateCodeBlockByIndex(t,e,i))}t=(t=fixTextCodeBlocks(replaceGSUUIDs(t)).text).replace(s,""),t=removeCodeBlockMarkers(t);s=isSearchableMessage(t+=`
