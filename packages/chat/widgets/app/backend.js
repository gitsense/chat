...
require("dotenv").config();let os=require("os"),path=require("path"),GSC_HOME=null,fs=(process.env.GSC_HOME?GSC_HOME=process.env.GSC_HOME:os.homedir()&&(process.env.GSC_HOME=GSC_HOME=path.resolve(os.homedir(),".gitsense")),console.log("Intialized GSC HOME: "+GSC_HOME),require("fs")),Anthropic=require("@anthropic-ai/sdk"),OpenAI=require("openai"),Cerebras=require("@cerebras/cerebras_cloud_sdk"),crypto=require("crypto"),sleep=require("util").promisify(setTimeout),{join,dirname}=require("path"),existsSync=require("fs").existsSync,ChatManager=require("@gitsense/gscb-git-tools").ChatManager,{init:initInit,models,providers,trees,readOptionsFile}=require("./init.js"),getDBPath=require("./db.js").getDBPath,{connect,closeAsync,allAsync,runAsync,prepareAsync,stmtFinalizeAsync,stmtRunAsync}=require("./sqlite.js"),Prompts=require("./prompts.js").Prompts,{search,generateSearchSystemPrompt,getSearchUserInstruction,getSearchHelp}=require("./components/search/backend/gitsense"),streamErrorResponse=require("./streamUtils").streamErrorResponse,buildAnalyzeMenuOptions=require("./components/chat-builder/utils/analyzeMenuBuilder").buildAnalyzeMenuOptions,SUGGEST_CHAT_TITLE=require("./prompt-templates.js").SUGGEST_CHAT_TITLE,prepareIsolatedTree=require("./components/chat-builder/utils/treesUtils").prepareIsolatedTree,{extractCodeBlocks,fixTextCodeBlocks,removeCodeBlockMarkers,formatWithLineNumbers,removeLineNumbers,updateCodeBlockByIndex,deleteCodeBlockByIndex,getChatTemplateMessages:_getChatTemplateMessages,estimateTokens,isToolBlock,parseToolBlock,parseTraceableCodeHeaders}=require("@gitsense/gsc-utils"),AnalyzerUtils=require("@gitsense/gsc-utils").AnalyzerUtils,{DropzoneHandler,SSEStreamHandler}=require("./components/import-data/backend"),GSCCodeService=require("./components/cli-bridge/backend").GSCCodeService,ContractService=require("./components/cli-contract/backend").ContractService,ManifestService=require("./components/metadata-insights-builder/backend").ManifestService,ChatService=require("./ChatService"),MessageService=require("./MessageService"),{ChatService:ClaudeCodeChatService,IntentWorkflowService:ClaudeCodeIntentWorkflowService}=require("./components/claude-code/backend"),SYSTEM_ROLE="system",USER_ROLE="user",ASSISTANT_ROLE="assistant",DEFAULT_GROUP_TYPE="regular",DEFAULT_PROMPT_TYPE="system",DEFAULT_CHAT_OWNER="everyone",DEFAULT_CHAT_TYPE="regular",DEFAULT_MSG_TYPE="regular",DEFAULT_CHAT_VISIBILITY="public",DEFAULT_MSG_VISIBILITY="public",STREAM_TIMEOUT=3e4,MAX_OUTPUT_TOKENS=4e3,ANALYZERS_BASE_PATH=path.resolve(GSC_HOME,"data","analyzers"),ENTERPRISE_BATCH_NOT_INSTALLED="Enterprise batch component not installed",dropzoneHandler=new DropzoneHandler(GSC_HOME),importDataStreamHandler=new SSEStreamHandler,gscCodeService=new GSCCodeService(GSC_HOME),manifestService=null,messageService=null,enterpriseBatch=null,contractService=null,intentWorkflowService=null,widget=null,claudeCodeAvailable=!1;async function init(e){widget=e;var e=getDBPath(),t=connect(e);try{await initInit(t);try{await(enterpriseBatch=require("@gitsense/gsc-enterprise-batch")).initialize({gscHome:GSC_HOME}),console.log("Enterprise batch processing installed")}catch(e){e?.message?.includes("Cannot find module")||console.log(e),console.log("Enterprise batch processing not installed")}try{var a=await ClaudeCodeChatService.checkAvailability();(claudeCodeAvailable=a.available)?console.log("Claude code is available"):console.log("Claude Code is not available: "+a.reason)}catch(e){console.log(e),console.log("Failed to determine if Claude Code is available")}chatService=new ChatService(e),messageService=new MessageService(e),contractService=new ContractService,intentWorkflowService=new ClaudeCodeIntentWorkflowService(chatService,messageService,contractService)}catch(e){console.log(e)}finally{await closeAsync(t)}}async function deleteData(e){var t=e.query.action;return"delete-analyzer"===t?deleteAnalyzer(e.query):"delete-chat"===t?deleteChat(e.query):"delete-chat-message"===t?deleteChatMessage(e.query):"delete-gsc-code"===t?deleteGSCCode(e.query):{status:"failed",data:{action:"Unrecognized action "+t}}}async function getData(e){var t,a,r=e.query.action;try{if("get-chat"===r)return await getChat(e.query);if("get-chat-ids"===r)return await getChatIds(e.query);if("get-chat-title-suggestion"===r)return await getChatTitleSuggestion(e.query);if("get-chats-in-trees"===r)return await getChatsInTrees(e.query);if("get-git-blob-chat-messages"===r)return await getGitBlobChatMessages(e.query);if("get-chat-analysis-messages"===r)return await getChatAnalysisMessages(e.query);if("get-chat-metadata"===r)return await getChatMetadata(e.query);if("get-cli-contract-info"===r)return await getCLIContractInfo(e.query);if("get-cli-contract-launch-list"===r)return await getCLIContractLaunchList(e.query);if("get-gsc-code-status"===r)return await getGSCCodeStatus(e.query);if("get-tiny-overview-chat-purpose"===r)return await getTinyOverviewChatPurpose(e.query);if("get-git-ref-chat-by-family-member"===r)return await getGitRefChatByFamilyMember(e.query);if("get-analyze-chat-menu-options"===r)return getAnalyzeChatMenuOptions(e.query);if("get-analyzers"===r)return getAnalyzers(e.query);if("get-analyzer-detail"===r)return getAnalyzerDetail(e.query);if("get-analyzer-schema"===r)return getAnalyzerSchema(e.query);if("get-chat-template-messages"===r)return getChatTemplateMessages(e.query);if("get-search-help"===r)return getSearchHelp(e.query);if("get-search-user-instructions"===r)return getSearchUserInstruction(e.query);if("get-options"===r)return getOptions();if("generate-search-system-prompt"===r)return t=readOptionsFile(),await generateSearchSystemPrompt(e.query,t);if("search"===r)return a=e.query?.query.includes("profile:git-nav")?readOptionsFile():null,await search(e.query,a);try{if("get-batch-group-chat-messages"===r){if(!enterpriseBatch)return{status:"failed",error:ENTERPRISE_BATCH_NOT_INSTALLED};var{"batch-job-id":s,"group-number":n}=e.query;try{s=parseInt(s),n=parseInt(n);return{success:!0,messages:await enterpriseBatch.getBatchGroupChatMessages(s,n)}}catch(e){return console.error(e),{success:!1,error:e.message}}}else if("get-batch-job-details"===r||"get-batch-job-details-with-worker-status"==r){if(!enterpriseBatch)return{status:"failed",error:ENTERPRISE_BATCH_NOT_INSTALLED};var i="get-batch-job-details"===r;try{var{"batch-job-id":o,"last-updated":c}=e.query,o=parseInt(o),l=await enterpriseBatch.getBatchJobDetails(o,c);return i?{success:!0,details:l}:{success:!0,data:{batchJobDetails:l,workerStatus:await enterpriseBatch.getWorkerStatus()}}}catch(e){return console.error(e),{success:!1,error:e.message}}}else if("get-scheduable-batch-providers"===r)return enterpriseBatch?{success:!0,providers:await enterpriseBatch.getScheduableBatchProviders()}:{success:!1,error:ENTERPRISE_BATCH_NOT_INSTALLED}}catch(e){return console.log(e),{success:!1,error:"Server side error"}}}catch(e){return console.error(e),{status:"failed",error:e.message}}return{status:"failed",data:{action:"Unrecognized action "+r}}}async function postData(e){var t=e.body.action;if("execute-gsc-cli-contract-launch"===t)try{var a=JSON.parse(e.body.payload);return await contractService.executeLaunch(a)}catch(e){return console.log(e),{success:!1,error:"Server side error"}}else{if("new-chat"===t)return newChat(e.body);if("new-chat-message"===t)return newChatMessage(e.body);if("new-chat-tree"===t)return newChatTree(e.body);if("intent-workflow-get-turn-stream"===t){var{"message-id":a,"contract-uuid":r,authcode:s,turn:n}=e.body;try{return await intentWorkflowService.getTurnStream({messageId:a,contractUuid:r,authcode:isNaN(s)?null:parseInt(s),turn:n})}catch(e){return console.log(e),{success:!1,error:"Server side error"}}}else if("intent-workflow-prepare-turn"===t)try{var{"message-id":i,"contract-uuid":o,authcode:c,"turn-type":l,intent:d,model:u}=e.body;return await intentWorkflowService.prepareTurn({messageId:i,contractUuid:o,authcode:c,turnType:l,intent:d,model:u})}catch(e){return console.log(e),{success:!1,error:"Server side error"}}else if("intent-workflow-validate-context-files"===t)try{var{"message-id":m,turn:h,"contract-uuid":g,authcode:p}=e.body;return await intentWorkflowService.validateContextFiles({messageId:m,contractUuid:g,authcode:p,turn:h})}catch(e){return console.log(e),{success:!1,error:"Server side error"}}else if("publish-manifest"===t)try{var{manifest:y,"publishing-context":f}=e.body;return await ManifestService.publishManifest(y,f)}catch(e){return console.log(e),{success:!1,error:"Server side error"}}else if("validate-mapped-workspace"===t)try{var{uuid:E,authcode:_,messageId:w}=e.body;return await contractService.validateMappedWorkspace(E,w,_)}catch(e){return console.log(e),{success:!1,error:"Server side error"}}}if("cancel-batch-job"===t){if(!enterpriseBatch)return{status:"failed",error:ENTERPRISE_BATCH_NOT_INSTALLED};a=e.body["batch-job-id"];try{return await enterpriseBatch.cancelBatchJob(parseInt(a))}catch(e){return console.log(e),{status:"failed",error:e}}}else if("create-batch-job"===t){if(!enterpriseBatch)return{status:"failed",error:ENTERPRISE_BATCH_NOT_INSTALLED};var{batchType:r="unknown",batchGroups:s,batchOptions:n,analyzerId:i,modelName:o,processType:c="realtime",triggerChatId:l,temperature:d=0}=e.body;try{return{success:!0,...await enterpriseBatch.createBatchJob(r,s,{modelName:o,temperature:d},c,{analyzerId:i,triggerChatId:l,metadataQuery:n?.metadataQuery})}}catch(e){return console.error(e),{success:!1,error:e.message}}}else if("create-gsc-code"===t){var{chatId:u,chatTitle:m,timeoutMinutes:g,parentMessageId:p,maxOutputSizeMB:h}=e.body;try{return gscCodeService.createGSCCode({chatId:u,chatTitle:m,timeoutMinutes:g,parentMessageId:p,maxOutputSizeMB:h})}catch(e){return console.error(e),{success:!1,error:"Server side error"}}}return{status:"failed",data:{action:"Unrecognized action "+t}}}async function putData(e){var t=e.body.action;if("clone-analyzer"===t)return cloneAnalyzer(e.body);if("save-analyzer"===t)return saveAnalyzer(e.body);if("save-import-data-files"===t)return saveImportDataFiles(e.body);if("update-chat"===t)return updateChat(e.body);if("update-chats"===t)return updateChats(e.body);if("update-chat-message"===t)return updateChatMessage(e.body);if("update-chat-analysis-messages"===t)return updateChatAnalysisMessages(e.body);if("update-chat-name"===t)return updateChatName(e.body);if("update-event-status"===t)try{return await contractService.updateEventStatus(e.body),{success:!0}}catch(e){return console.log("Failed to update events table: ",e),{success:!1,message:"Server side error"}}else if("reset-chat-message"===t)return resetChatMessage(e.body);if("reset-batch-job-group"===t){if(!enterpriseBatch)return{status:"failed",error:ENTERPRISE_BATCH_NOT_INSTALLED};var{"batch-job-id":e,"group-number":a}=e.body;try{return await enterpriseBatch.resetBatchGroup(parseInt(e),parseInt(a))}catch(e){return console.error(e),{success:!1,message:"Failed to reset batch group: "+e.message}}}return{status:"failed",data:{action:"Unrecognized action "+t}}}async function stream(u,m,o){let{"message-id":e,"assistant-message-id":C,provider:s,"storage-id":c,task:t,command:l,authcode:a,"chat-id":d,"contract-uuid":r,simulate:B,"stream-delay":n,fields:P,files:z}=o;if(c)return importDataStreamHandler.handleStreamRequest(u,m);if("execute-command"===t&&r&&a&&l)return contractService.executeCommandStream({uuid:r,authcode:a,command:l,res:m});if("watch-events"===t&&r&&d)return contractService.watchEvents({uuid:r,chatId:d,res:m});if("intent-workflow-create-context"===t)await intentWorkflowService.createContextMessage({messageId:e,files:z,contractUuid:r,authcode:a},m);else if("intent-workflow-delete-turn"===t)await intentWorkflowService.deleteTurn({messageId:e,contractUuid:r,authcode:a,res:m});else if("intent-workflow-monitor"===t){if(n)try{n=parseInt(n)}catch(e){n=void 0,console.warn(`[backend::intent-workflow-monitor] Stream delay is not a number. Ignoring passed value ${n}.`)}await intentWorkflowService.monitorSession({messageId:e,contractUuid:r,authcode:a,simulate:B,streamDelay:n,res:m})}else if("intent-workflow-retry-turn"===t)await intentWorkflowService.retryTurn({messageId:e,contractUuid:r,authcode:a,res:m});else if("intent-workflow-start-session"===t)await intentWorkflowService.startSession({messageId:e,contractUuid:r,authcode:a,res:m});else if("intent-workflow-stop-session"===t)await intentWorkflowService.stopSession({messageId:e,contractUuid:r,authcode:a,res:m});else if("intent-workflow-watch-meta"===t)await intentWorkflowService.watchMeta({messageId:e,fields:P,res:m});else{e&&(C=e);let l=connect(getDBPath()),d=null;try{if(!(d=await getMessage(l,C)))return streamErrorResponse(m,`No assistant message to be chat completed with the ID ${C} found`);let{message:e,model:t,real_model:a,temperature:o}=d;if(null!==e)return streamErrorResponse(m,"Chat message has already been completed");var i=await getChatPrivate(l,d.chat_id);if(!i)return streamErrorResponse(m,"No chat associated with the assistant message id "+C);var W=i.uuid;if("Claude Code"===s)try{var G,q,Y=await getMessage(l,d.parent_id);return Y?(q=(G=d.real_model||d.model).split(" - ").pop()?.toLowerCase()||null)?await ClaudeCodeChatService.executeStream({chat:i,userMessage:Y,model:q,res:m,onBeforeDone:async(e,t,a)=>(await H(e,a,t),{success:!0}),onClose:async(e,t)=>{await H(e,t)}}):streamErrorResponse(m,"Unable to determine model alias from "+G):streamErrorResponse(m,`No parent message with the ID ${d.id} found`)}catch(e){return void console.log(e)}finally{console.log("[Main Backend] Closing database connnection..."),await closeAsync(l)}var h=readOptionsFile(),g=a||t,p=!!g.match(/^Fake/),y=!!g.match(/Notes/i);let c=null;if(p||y||s){var j=(c=p||y?{}:providers[s.toLowerCase()]).apiKeyName,f=j?process.env[j]:"";if(p||y||f){var E=models[g]?.providers||null;if(p||y||E){var _="number"==typeof h.maxChatCompletionSize?h.maxChatCompletionSize:-1;let t="number"==typeof h.maxOutputTokens?h.maxOutputTokens:MAX_OUTPUT_TOKENS,r=null,n=null,i=null;if(p||y)i=g;else for(let e=0;e<E.length;e++){var w=E[e];if(w.name.toLowerCase()===s.toLowerCase()){i=w.modelId,w.maxOutputTokens&&(t=w.maxOutputTokens),w.thinkingBudget&&(r=isNaN(w.thinkingBudget)?0:w.thinkingBudget),w.includeThoughts&&(n=w.includeThoughts||!1);break}}if(i){global.gschat||(global.gschat={streaming:{}});var S=W+"::"+g+"::"+o,J=global.gschat.streaming[S];if(J){var{startedAt:V,updatedAt:Z}=J,K=Date.now()-V,X=Date.now()-Z;if(X<STREAM_TIMEOUT)return void m.status(400).send({status:"failed",data:`Streaming in progresss. Started ${K}ms ago and last updated ${X}.`});console.warn(`The stream ${S} has not updated in over ${STREAM_TIMEOUT}ms`),delete global.gschat.streaming[S]}var v={startedAt:Date.now(),updatedAt:Date.now()};global.gschat.streaming[S]=v,console.log("Created streaming lock for "+S);try{var A=(await getMessageLineage(l,C)).filter(e=>"public"===e.visibility),Q=A.map(e=>e.message),ee=JSON.stringify(Q).length;if(-1!==_&&_<ee)await k(m,`Total chat completion payload message size (${ee}) exceeds the maximum allowed character limit of ${_} characters.`);else{m.setHeader("Content-Type","text/event-stream"),m.setHeader("Cache-Control","no-cache"),m.setHeader("Connection","keep-alive"),m.flushHeaders();var te="anthropic"===s.toLowerCase();if(p){var ae=g;var re=A;let a=[],e=(re.forEach((e,t)=>{t!==re.length-1&&({role:t,message:e}=e,""!==e)&&a.push({role:t,content:e})}),Date.now()),t="This is a Fake LLM response.",r={content:t,done:!1};m.write(`id: ${e}
event: message
data: ${JSON.stringify(r)}

`),m.flush?.(),r={content:"",done:!0},m.write(`id: ${e}
event: message
data: ${JSON.stringify(r)}

`),m.flush?.(),m.write(`id: ${e}
event: complete
data: {}

`),m.end(),await H(t,ae);void await 0}else if(y){var se=g;var ne=A;let a=[],e=(ne.forEach((e,t)=>{t!==ne.length-1&&({role:t,message:e}=e,""!==e)&&a.push({role:t,content:e})}),Date.now()),t="This is a GitSense Notes response.",r={content:t,done:!1};m.write(`id: ${e}
event: message
data: ${JSON.stringify(r)}
...
`),m.end(),await H(t,se);void await 0}else{if(te){var ie=A,T=v,N=g,oe=o,I=i,ce=t,le=f,de=S;let a=null,r=[];ie.forEach((e,t)=>{t!==ie.length-1&&({role:t,message:e}=e,""!==e)&&(t===SYSTEM_ROLE?a=e:r.push({role:t,content:e}))});var R={messages:R=populateMessages(N,r),model:I,temperature:oe,max_tokens:ce,stream:!0};a&&(I=populateMessages(N,[{content:a}]),R.system=I[0].content);try{u.setTimeout(3e4);var D=Date.now();let e=!1;u.on("close",()=>{e=!0});var O,M,ue,me=new Anthropic({apiKey:le}),b=(console.log(`Starting stream ${de}...`),{startedAt:(new Date).getTime()}),he=await me.messages.create(R);T.createdAt=Date.now(),T.payload=R;let t=!1,a="";for await(O of he){if(T.updatedAt=Date.now(),e)break;if("content_block_delta"===O.type&&(M=O.delta.text||"")&&(a+=M,ue={content:M,done:!1},m.write(`id: ${D}
event: message
data: ${JSON.stringify(ue)}

`),m.flush?.()),"message_stop"===O.type){console.log("Finished streaming "+de),m.write(`id: ${D}
event: message
data: {"done": true}

`),m.flush?.(),b.done=!0,b.stoppedAt=(new Date).getTime(),t=!0;break}}t||(b.incomplete=!0,b.stoppedAt=(new Date).getTime(),a+="\n\n---\nWarning: Incomplete response"),e||(m.write(`id: ${D}
event: complete
data: {}

`),m.end()),await H(a,N,b)}catch(e){console.error("Streaming error: "+e),await k(m,e.message),m.end()}}else{var ge=c,pe=A,L=v,U=g,ye=o,fe=i,Ee=t,_e=r,we=n,Se=f,ve=S;let a=[],s=(pe.forEach((e,t)=>{t!==pe.length-1&&({role:t,message:e}=e,""!==e)&&a.push({role:t,content:e})}),populateMessages(U,a));try{u.setTimeout(3e4);let t=!1;u.on("close",e=>{t=!0});var x={model:fe,temperature:ye,messages:s,stream:!0,max_tokens:Ee},Ae=((_e||we)&&(x.extra_body={google:{thinking_config:{thinking_budget:0,include_thoughts:we}}}),"cerebras"===ge.name.toLowerCase()),Te=(Ae&&(x.reasoning_format="hidden"),Ae?new Cerebras({apiKey:Se}):new OpenAI({apiKey:Se,baseURL:ge.baseURL})),$={startedAt:(new Date).getTime()};let e=null;try{console.log(`Starting stream ${ve}...`),e=await Te.chat.completions.create(x)}catch(e){return void await(console.error("Failed to create a stream:",e),e.message.includes("status code")&&(e.message+="\n\nPlease look up the error code with the provider to determine the problem."),await k(m,e.message,U),void m.end())}L.createdAt=Date.now(),L.payload=x;var F,Ne=Date.now();let a=!1,r="";for await(F of e){if(L.updatedAt=Date.now(),t)break;var Ce=F.choices[0]?.delta?.content||"",Ie=F.choices[0]?.finish_reason,Re=Ie&&"null"!==Ie&&"stop"===Ie,De={content:Ce,done:Re};if(Ce&&(r+=Ce),m.write(`id: ${Ne}
event: message
data: ${JSON.stringify(De)}

`),m.flush?.(),Re){console.log("Finished streaming "+ve),""===r.trim()&&(r="### WARNING\n\nThe LLM stopped without returning a response. If a response was expected, please try sending a `Try again` message."),$.done=!0,$.stoppedAt=(new Date).getTime(),a=!0;break}}a||($.incomplete=!0,$.stoppedAt=(new Date).getTime(),r+="\n---\nWarning: Incomplete response"),t||(m.write(`id: ${Ne}
event: complete
data: {}

`),m.end()),await H(r,U,$)}catch(e){console.error("Streaming error:",e),await k(m,e.message),m.end()}}await 0}}}finally{console.log("Deleting streaming lock "+S),delete global.gschat.streaming[S]}}else m.status(500).send({status:"failed",data:`Server side error. No model identifier for ${g} by ${s} found.`})}else m.status(400).send({status:"failed",data:"No providers for the LLM "+g})}else await k(m,`No ${s} API key defined. Please define to chat with ${g}.`)}else m.status(400).send({status:"failed",data:{provider:"Missing provider information"}})}finally{await closeAsync(l)}async function k(e,t,a){console.log("Stream error message",t);try{try{e.setHeader("Content-Type","text/event-stream"),e.setHeader("Cache-Control","no-cache"),e.setHeader("Connection","keep-alive"),e.flushHeaders()}catch(e){}var r=Date.now(),s={content:t,done:!1};e.write(`id: ${r}
event: message
...
`),e.end(),await H(t,a),console.log("saved response")}catch(e){console.log(e.message)}}async function H(N,e,t){var a=N;let r=extractCodeBlocks(N,{silent:!0}).blocks||[];for(let e=0;e<r.length;e++){var s=r[e];s.index=e,s.header?.["Block-UUID"]&&(s=removeLineNumbers(s.headerText+"\n\n\n"+s.content),N=updateCodeBlockByIndex(N,e,s))}var n=(new Date).toISOString(),n=(N=(N=N.replace(/{{UTC-TIME}}/g,n)).replace(/{{MODEL-NAME}}/g,e),N=fixTextCodeBlocks(replaceGSUUIDs(N)).text,new RegExp("Authored by [^\n]+\n*$")),n=(N=N.replace(n,""),N=removeCodeBlockMarkers(N),isSearchableMessage(N+=`

Authored by LLM ${e||"N/A"} at `+(new Date).toUTCString())),e=crypto.createHash("sha256").update(N+d.created_at).digest("hex");let i=`
            UPDATE
                messages
            SET
                message = ?,
            ...
            WHERE
                id = ?
        `;a=[N,a,e];t&&a.push(JSON.stringify(t)),n||a.push(JSON.stringify({searchable:!1})),a.push(C);try{await runAsync(l,i,a)}catch(e){throw new Error(`Failed to update chat message ${C}:
${i}
`+e)}if((r=(extractCodeBlocks(N,{silent:!0})||{}).blocks||[]).length){let v=parseTraceableCodeHeaders(N),A=!1,T=new Set;if((r=r.filter((c,l)=>{var{type:d,header:u,metadata:m,content:h}=c;if(u||m){c.index=l;var g="code"===d,p="patch"===d;let{Component:e,Description:t,Language:a,"Block-UUID":r,"Parent-UUID":s,"Source-Block-UUID":n,"Target-Block-UUID":i,"Target-Version":o}=g?u:m;if(!(g&&!u.Version||p&&!o)){var y,f,E,_,w,S=(g?u.Version:o).split(/\./);if(S&&3===S.length){if(!S.find(e=>isNaN(e)))return g?(w=r,r=crypto.randomUUID(),console.log(`Replaced Block-UUID ${w} with `+r)):(w=i,i=crypto.randomUUID(),console.log(`Replaced Target-Block-UUID ${w} with `+i)),w=h.trim(),{isTraceable:w,isNewVersion:y,currentParentUUID:f,newParentUUID:E,newBlockUUID:_}=(c.hash=crypto.createHash("sha256").update(h).digest("hex"),c.trimmedHash=crypto.createHash("sha256").update(w).digest("hex"),c.component=e||"None Provided",c.uuid=r||n+":"+i,c.parentUuid=s||n||"N/A",c.version=S.join("."),c.language=a||"None Provided",c.description=t||"None Provided",v[c.index]?.data||{}),w&&g?(y&&c.parentUuid!==E&&(console.log("WARNING: Parent UUID is not the same as the New Parent UUID as defined in the traceable code header text"),c.parentUuid=E),w=c.headerText.replace(/Block-UUID: [\w-]+\n/g,`Block-UUID: ${c.uuid}
...
`),N=updateCodeBlockByIndex(N,l,g),A=!0,T.add(c.uuid||i)),c.upsertParams=[C,d,c.uuid,"N/A"===c.parentUuid?null:c.parentUuid,c.component,c.description,c.language,c.hash,c.trimmedHash,h.length,S[0],S[1],S[2],JSON.stringify(u||m),h],c;console.warn(`Found the non number "${nonNumber}" in the version array. Ignoring code block.`)}else console.warn(`Expecting version array to contain a major, minor and patch value. Found "${JSON.stringify(S)}" instead. Ignoring code block.`)}}})).length){let u=await(async(e,a)=>{var r=`
            ...
            `;try{var s=await allAsync(e,r,a);let t={};return s.forEach(e=>{t[e.uuid]=e}),t}catch(e){throw new Error(`Failed to update chat message ${C}:
${i}
`+e)}})(l,[...T]);if(r.forEach(e=>{var t,a,r,s,n,i,o,c,l,d=u[e.uuid];d&&({type:t,uuid:a,parentUuid:l,component:r,version:s,language:n}=e,{parent_uuid:d,component:i,language:o,version:c}=d,d===l&&i===r&&c===s&&o===n||(A=!0,d=crypto.randomUUID(),"code"===t?(console.log(`Changing block uuid from ${a} to `+d),e.upsertParams[2]=d,l=e.headerText.replaceAll(a,d)+"\n\n\n"+e.content,N=(N=updateCodeBlockByIndex(N,e.index,l)).replaceAll("\n\nAuthored by LLM",`
...
Authored By LLM`)):"patch"===t?console.log("TODO: We need to change the uuid for patch code block"):console.log("Unrecognized code block type "+t)))}),A){a[0]=N,a[2]=crypto.createHash("sha256").update(N+d.created_at).digest("hex");try{await runAsync(l,i,a)}catch(e){throw new Error(`Failed to update chat message ${C}:
${i}
`+e)}}var o=`
...
`+e.message),new Error("Server side error")}})(),c=await(async()=>{var t=`
            ...
            WHERE
                ...
                c.id IN (${s.join(",")}) AND
                m.type = '${r}-overview'
        `;try{var e=await allAsync(n,t);let o={};return e.forEach(e=>{var{id:e,type:t,parent_id:a,chat_id:r,content:s,meta:n,updated_at:i}=e;o[r]={id:e,type:t,parent_id:a,chat_id:r,content:s,meta:JSON.parse(n),updated_at:i}}),o}catch(e){throw console.error(`Failed to execute ${t}:
...
`+e.message),new Error("Server side error")}})(v,null,C))?A.id:e)||await insertGroup(v,H,C);var P=n||0;let t=null;if(d)if(l){var I=new Prompts,R=I.computeHash(f,l,d);if(!(t=await getPrompt(v,null,R))||!t.id){var D=await I.insert(v,null,f,R,l,d);if(!D)throw new Error("No prompt created");t={id:D.id,content:D.prompt}}}else d={id:0,content:d};else t=await getPrompt(v,null,null,f,l);if(!t||void 0===t.id)throw new Error("No system prompt id defined");var O,z=w[0]?.created_at?w[0].created_at:null,W=w[0]?.updated_at?w[0].updated_at:null,M=await insertChat(v,a,DEFAULT_CHAT_VISIBILITY,T,B,N,0,P,e,t.id,h,S,s?0:1,E,z,W);w.length?(O=w[0]).role===SYSTEM_ROLE&&("<replace with system message>"===O.message||d&&l)&&(O.message=t.content):(w.push({role:SYSTEM_ROLE,message:t.content}),h.match(/Notes/)?w.push({role:ASSISTANT_ROLE,message:p}):(w.push({role:USER_ROLE,message:p}),w.push({role:ASSISTANT_ROLE,message:y})));let u=0,m=null;for(let d=0;d<w.length;d++){let{id:e,role:t,message:a,content:r,type:s,visibility:n,metadata:i,meta:o,created_at:c,updated_at:l}=w[d];if(t===SYSTEM_ROLE){var G=(await insertMessage(v,s||DEFAULT_MSG_TYPE,n||DEFAULT_MSG_VISIBILITY,M,u,null,null,null,null,t,null!=r?r:null!=a?a:null,i||o,e,c,l)).id;u=G}else if(t===USER_ROLE){var q=(await insertMessage(v,s||DEFAULT_MSG_TYPE,n||DEFAULT_MSG_VISIBILITY,M,u,null,null,null,null,t,null!=r?r:null!=a?a:null,i||o,e,c,l)).id;u=q}else{if(t!==ASSISTANT_ROLE)return{status:"failed",code:400,data:"Invalid message role "+t};var Y=(await insertMessage(v,s||DEFAULT_MSG_TYPE,n||DEFAULT_MSG_VISIBILITY,M,u,1,h,_,g,t,null!=r?r:null!=a?a:null,i||o,e,c,l)).id;m=u=Y}}var b,j,L,U,J,x,$,{type:F="",options:k={}}=r||{},V=(F.match(/no error analysis/i),F.match(/validate/i)),Z=F.match(/compare/i);return F.match(/samples/i)?({samples:b,summarize:j,summarizer:L}=k,await newCompareSamplesAnalysis(v,m,b,L)):Z?({models:U,summarize:J,summarizer:x}=k,await newCompareModelsAnalysis(v,m,U,x)):V&&($=k.models,await newValidateAnalysis(v,m,$)),await getChat({id:M})}finally{await closeAsync(v)}}async function getChatDescendants(e,t,a=1e4){var r=`
        WITH RECURSIVE matches AS (
            SELECT
                c.id,
                c.type,
...
`+e.message),new Error("Server side error")}return n||[]}async function insertMessage(e,t,a,r,s,n,i,o=null,c,l,d=null,u=null,m,h,g){var p=(new Date).toISOString(),p=(h=h||p,g=g||p,crypto.createHash("sha256").update(d+h).digest("hex")),y=`
    ...
    `,f=!d||isSearchableMessage(d),f=(u&&!f?u.searchable=!1:f||(u={searchable:!1}),[t,a,r,s,s,p,n,i,o,c,l,d,u?JSON.stringify(u):null,m||null,h,g]);let E=null;try{if(!(E=await runAsync(e,y,f)).changes)throw new Error("No changes after insert message")}catch(e){throw console.log("Failed to insert the following message:"),console.log(JSON.stringify({params:f},null,2)),new Error(`Failed to insert message:
${y}
${JSON.stringify(f)}
`+e)}return getMessage(e,E.lastInsertRowid)}async function getAnalysis(e,t,a,r,s,n,i,o=0,c=1){let l=`
        SELECT
            id,
            message_ids,
            analysis_ids,
...
${c}
`+e)}}async function resetChatMessage(e){var e=e.id,t=connect(getDBPath()),a=await getMessage(t,e);if(!a)return{status:"failed",message:`No message with the ID '${e}' found`};var a=`
        UPDATE
            messages
        SET
            message = NULL,
            ${"regular"===a.type?"meta=null,":""}
    ...
    `,r=`
        ...
        WHERE
            parent_id = ?
    `;try{try{if(!(await runAsync(t,a,[e])).changes)return{status:"failed",data:"No matching message. Make sure the id and original message is correct."}}catch(e){throw new Error(`Failed to update chat message:
${a}
`+e)}try{await runAsync(t,r,[e])}catch(e){throw new Error(`Failed to detach chat message:
${r}
`+e)}return{status:"success"}}finally{await closeAsync(t)}}async function cloneAnalyzer(e){var{"analyzer-id":e,"new-analyzer-name":t,label:a,description:r}=e;try{var s=await AnalyzerUtils.cloneAnalyzer(ANALYZERS_BASE_PATH,e,t,{label:a,description:r});return s.success?s:{success:!1,error:cloneResult.message}}catch(e){return console.error(e),{success:!1,data:"Server side error"}}}async function saveAnalyzer(e){var{"analyzer-id":e,instructions:t}=e;try{var a=await AnalyzerUtils.saveConfiguration(ANALYZERS_BASE_PATH,e,t);return a.success?{status:"success"}:{status:"failed",data:a.message}}catch(e){return console.log(e),{status:"failed",data:e.message||e}}}async function updateAnalyzer(e){var{"analyzer-id":e,name:t,label:a,description:r}=e;try{var s=AnalyzerUtils.updateAnalyzer(ANALYZERS_BASE_PATH,e,{name:t,label:a,description:r});return s.success?{success:!0,data:s.message}:{success:!1,error:s.message}}catch(e){return console.log(e),{status:"failed",data:e}}}async function updateChat(e){var{uuid:e,name:t,parentId:a}=e,r=connect(getDBPath()),s=[],n=[],t=(t&&(s.push("name = ?"),n.push(t)),null!=a&&(s.push("parent_id = ?"),n.push(a)),n.push(e),`
        UPDATE chats
        SET ${s.join(",")}
        WHERE uuid = ?
...
`+e)}var p=i||d.meta,y=(c&&!isSearchableMessage(c)?p.searchable=!1:i&&(p.searchable=d.meta?.searchable||null),`
            ...
            WHERE
                id = ?
                ${null==a?"":"AND message = ?"}
        `);try{let e=[];r&&e.push(c),s&&e.push(s),n&&e.push(n),(i||c)&&e.push(JSON.stringify(p)),"number"==typeof o&&0<=o&&e.push(o),e.push(t),null!=a&&e.push(a);var f=await runAsync(l,y,e);return f.changes?(f.oldMessage=d,f.newMessage=await getMessage(l,t),{success:!0,...f}):{success:!1,error:"No matching message. Make sure the id and original message is correct."}}catch(e){return console.error(`Failed to update chat:
${y}
...
${i}
`+e),{success:!1,error:"Server side error"}}finally{await closeAsync(o)}}async function updateChatAnalysisMessages(a){var r=connect(getDBPath());try{let{"analyzer-id":n,analyses:e}=a;var s=e.map(e=>e.chatId),m=await(async(e,t)=>{let a=`
            SELECT
                id,
                main_model,
                meta
...
`+e)}var p=`
                ...
                WHERE
                    id = ?;
            `;let t=null;try{t=await prepareAsync(r,p);for(let e=0;e<l.length;e++){var{id:y,message:f,meta:E}=l[e];await stmtRunAsync(t,[f,JSON.stringify(E),y])}}catch(e){throw new Error(`Failed to update message:
${p}
`+e)}finally{if(t)try{await stmtFinalizeAsync(t)}catch(e){console.error("Failed to finalize statement:",e)}}}if(d.length){var _=`
                INSERT INTO messages(
                    type,
                    deleted,
                    visibility,
            ...
            `;let t=null;try{t=await prepareAsync(r,_);for(let e=0;e<d.length;e++)await stmtRunAsync(t,d[e])}catch(e){throw new Error(`Failed to insert new message:
${_}
