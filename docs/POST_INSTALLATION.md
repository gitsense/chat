...
GitSense Chat provides a command-line administration tool, `gsc-admin`, to manage your LLM models and providers directly within the `chat-config.json` configuration file. This tool offers interactive prompts to guide you through the process.
...
*   Restoring your `chat-config.json` configuration from backups.
...
*   **Preview of New Order & Confirmation:**
    The tool will show a preview of how your `models` list will look in `chat-config.json` and ask for final confirmation.
```
...
**3. Completion:**
The tool will confirm that the model has been added to `chat-config.json`. Remember to restart your GitSense Chat application for the changes to take effect.
...
The `gsc-admin` tool automatically creates rolling backups of your `chat-config.json` file whenever changes are saved. You can use the `gsc-admin config restore` command to revert to a previous version of your configuration.

**Important:** Restoring a backup will **overwrite your current `chat-config.json` file**. The tool will create an "undo" backup of your current `chat-config.json` before proceeding with the restore.
